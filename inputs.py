...
def collect_inputs():
    ...
    if n_tx > 0:
        ...
        kva_raw = ask("  Transformer rating (kVA): ", "1000")
        kva = int(kva_raw.strip())

        print(f"  Enter dimensions for {kva}kVA / {kv}kV transformer:")
        tx_l = int(ask("  Length (mm): ", "2000"))
        tx_w = int(ask("  Width  (mm): ", "1500"))
        tx_h = int(ask("  Height (mm): ", "1800"))

        print(f"  -> Transformer set: {tx_l} x {tx_w} mm  (H={tx_h} mm)")
